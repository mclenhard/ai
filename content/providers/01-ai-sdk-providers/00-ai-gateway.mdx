...
- **hipaaCompliant** _boolean_
  ...
  Restricts routing requests to providers that are HIPAA compliant.

- **quotaEntityId** _string_

  An identifier for the entity to track spend quotas against. When set, the gateway enforces spend limits associated with this ID.

  Example: `quotaEntityId: 'team-abc-123'`
...
Set `hipaaCompliant` to true to ensure requests are only routed to providers
that are HIPAA compliant. When `hipaaCompliant` is `false` or not specified,
there is no enforcement of restricting routing.
